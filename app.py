...
def save_groups_data(df_to_save):
  ...
  try:
    ...
    ws.clear()
    ws.update(range_name="A1", values=values_to_write)
    st.cache_data.clear()
    return True
  except Exception as e:
    st.error(f"Ошибка сохранения: {e}")
...
def delete_group_data(row_index, full_df):
  """Удаляет указанную строку из DataFrame и перезаписывает таблицу."""
  if row_index is not None and row_index in full_df.index:
    df_updated = full_df.drop(index=row_index).reset_index(drop=True)
    return save_groups_data(df_updated)
  return False
...
def group_editor_dialog(row_data, row_index, full_df, dict_materik, dict_palas):
  ...
  with st.form("group_edit_form"):
    ...
    with b_col2:
      deleted = False
      if not is_new:
        deleted = st.form_submit_button("🗑️ Удалить группу", use_container_width=True, type="secondary")
    ...
    if deleted and not is_new:
      if delete_group_data(row_index, full_df):
